...
function getPINs(observed) {
  if (typeof observed !== 'string') return 'Invalid Input';

  if(observed.length === 1) return combos[observed];

  const foundPins = getPINs(observed.slice(1));

  return combos[observed[0]].reduce((prevs, combo) => {
    return [...prevs, ...foundPins.map(found => combo.concat(found))]
  }, [])
}

console.log(getPINs('1111'))
