...
// Input
  // A string, containing 1-8 digits 0-9
// Output
  // An array of strings,
    // each corresponding to a possible PIN that has digits on or adjacent to the observed input digits
...
function getPINs(observed) {
  // Create store array
    // Iterate through the input string
      // Push an array of options for the current digit into store
  const options = observed.split('').map(digit => findAdjacent(digit));
  // Iterate through options
  let output = ['']
  options.forEach(digitOptions => {
    output = appendDigit(output, digitOptions);
  });

  return output;
}

const appendDigit = (array, digitOptions) => {
  const output = [];
  // Iterate through digitOptions and append digit to another instance of each element in the array
...
const findAdjacent = digit => {
  digit = parseInt(digit);
  if (digit === 0) return  ['0','8'];
  if (digit === 1) return  ['1','2','4'];
  if (digit === 2) return  ['1','2','3','5'];
  if (digit === 3) return  ['2','3','6'];
  if (digit === 4) return  ['1','4','5','7'];
  if (digit === 5) return  ['2','4','5','6','8'];
  if (digit === 6) return  ['3','5','6','9'];
  if (digit === 7) return  ['4','7','8'];
  if (digit === 8) return  ['5','7','8','9','0'];
  if (digit === 9) return  ['6','8','9'];
}

// Test
//console.log(findAdjacent('8'));
// console.log(getPINs('8'));
// console.log(getPINs('11'));
// console.log(appendDigit(getPINs('1')[0],getPINs('1')[0]));
